...
- **DOM measurement and imperative DOM work in `afterNextRender()` / `afterEveryRender()`**, not `ngAfterViewInit` — these run at the right phase and are safe under
...
- **In an app still on Zone.js, run a high-frequency listener** (`scroll`, `mousemove`, `requestAnimationFrame`) inside `NgZone.runOutsideAngular`, or each event
