...
- **`[class.active]` / `[style.width.px]`** over `NgClass` / `NgStyle` — clearer and faster.
- **Extensibility via projection**: `<ng-content>` + named slots (`select=`), a caller-supplied template as an `input<TemplateRef>()` rendered by
  `*ngTemplateOutlet`, or a caller-supplied component via `NgComponentOutlet` — the Angular mechanism for the slot-vs-prop decision in
