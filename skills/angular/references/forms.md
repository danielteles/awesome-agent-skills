...
- **Reactive, typed forms** (`FormGroup`, `FormControl`) for anything past a single field — a template-driven form hides its model and is hard to test. Type
  every control: `new FormControl<string>('', { nonNullable: true })`; an untyped form loses every guarantee. Keep template-driven only for a trivial single
  input.
