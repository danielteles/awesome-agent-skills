...
- **No project-wide barrel.** A wide `index.ts` re-exporting everything breaks tree-shaking and invites import cycles (`architecture-and-design`, structure). A
  single feature's `index.ts` as its public boundary is fine.
