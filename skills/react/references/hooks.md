...
- **Top level, unconditional.** React identifies each hook by its call order within a render. A hook skipped by a condition or an early `return` shifts every later hook onto the wrong slot, so state and effects from one hook leak into another. This is why a hook cannot go in a condition, loop, nested function, event handler, `try`/`catch`, or a callback passed to `useMemo` / `useReducer` / `useEffect` — those run outside the render's hook sequence.
